...
public class ZeppOsAlarmsService extends AbstractZeppOsService {
    ...
    public void sendAlarm(final com.example.gr.device.model.Alarm alarm, final TransactionBuilder builder) {
    ...
    private void decodeAndUpdateAlarms(final byte[] payload) {
        ...
        final List<com.example.gr.database.entities.Alarm> dbAlarms = DBHelper.getAlarms(getSupport().getDevice());
        int numUpdatedAlarms = 0;

        for (com.example.gr.database.entities.Alarm alarm : dbAlarms) {
    ...
    private Alarm parseAlarm(final byte[] payload, final int offset) {
        final com.example.gr.database.entities.Alarm alarm = new com.example.gr.database.entities.Alarm();
