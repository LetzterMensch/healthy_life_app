...
public class DeviceHelper {
    ...
    public Set<GBDevice> getAvailableDevices(Context context) {
        ...
        if (miAddress.length() > 0) {
            GBDevice miDevice = new GBDevice(miAddress, "MI", null, null, DeviceType.MIBAND7);
    ...
    private List<GBDevice> getDatabaseDevices() {
        ...
        try (DBHandler lockHandler = ControllerApplication.acquireDB()) {
            ...
            GB.toast(ControllerApplication.getContext().getString(R.string.error_retrieving_devices_database), Toast.LENGTH_SHORT, GB.ERROR, e);
