...
public class BondingUtil {
    ...
    public static void tryBondThenComplete(BondingInterface bondingInterface, BluetoothDevice device, String macAddress) {
        ...
        if (bondState == BluetoothDevice.BOND_BONDED) {
            toast(bondingInterface.getContext().getString(R.string.pairing_already_bonded, device.getName(), device.getAddress()), Toast.LENGTH_SHORT, GB.INFO);
            //noinspection StatementWithEmptyBody
            if (ControllerApplication.getPrefs().getBoolean("enable_companiondevice_pairing", true) &&
            ...
            toast(bondingInterface.getContext(), bondingInterface.getContext().getString(R.string.pairing_in_progress, device.getName(), device.getAddress()), Toast.LENGTH_LONG, GB.INFO);
            return;
        }

        toast(bondingInterface.getContext(), bondingInterface.getContext().getString(R.string.pairing_creating_bond_with, device.getName(), device.getAddress()), Toast.LENGTH_LONG, GB.INFO);
        ...
        boolean companionPairingEnabled = ControllerApplication.getPrefs().getBoolean("enable_companiondevice_pairing", true) &&
                Build.VERSION.SDK_INT >= Build.VERSION_CODES.O;
