...
public class ZeppOsAlarmsService extends AbstractZeppOsService {
    ...
    private void decodeAndUpdateAlarms(final byte[] payload) {
        ...
        final List<com.example.gr.model.database.entities.Alarm> dbAlarms = DBHelper.getAlarms(getSupport().getDevice());
        int numUpdatedAlarms = 0;

        for (com.example.gr.model.database.entities.Alarm alarm : dbAlarms) {
    ...
    private Alarm parseAlarm(final byte[] payload, final int offset) {
        final com.example.gr.model.database.entities.Alarm alarm = new com.example.gr.model.database.entities.Alarm();
