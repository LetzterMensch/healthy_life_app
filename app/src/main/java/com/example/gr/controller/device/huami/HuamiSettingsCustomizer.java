...
import com.example.gr.controller.device.settings.DeviceSpecificSettingsCustomizer;
        ...
        for (HuamiVibrationPatternNotificationType notificationType : HuamiVibrationPatternNotificationType.values()) {
            ...
            if (tryPref != null) {
                tryPref.setOnPreferenceClickListener(preference -> {
                    ControllerApplication.deviceService(device).onSendConfiguration(tryPrefKey);
                    return true;
                });
            }
